...
function J = JacobianTEST(Tr,q,i_here,j_here)%i_here is link index, j_here is division (0 for joint)
...
if nargin==4
    if j_here==0
        ...
        nsig = Tr.CVRods{i_here}(j_here+1).nip; %j_here>1 is allowed only for soft links
    end
elseif nargin==3
    if Tr.VLinks(Tr.LinkIndex(i_here)).linktype=='s'
        nsig = 1;
        for j=1:Tr.VLinks(Tr.LinkIndex(i_here)).npie-1
            nsig = nsig+Tr.CVRods{i_here}(j+1).nip;
        end
    else
        nsig = 2; %joint and CM
    end
...
for i = 1:N
    ...
    Phi_here   = Tr.CVRods{i}(1).Phi;
    xi_star  = Tr.CVRods{i}(1).xi_star;
    ...
    for j = 1:Tr.VLinks(Tr.LinkIndex(i)).npie-1 %will run only if soft link
        ...
        lpf      = Tr.VLinks(Tr.LinkIndex(i)).ld{j};
        Xs       = Tr.CVRods{i}(j+1).Xs;
        nip      = Tr.CVRods{i}(j+1).nip;

        if Tr.Z_order==4
            Phi_Z1       = Tr.CVRods{i}(j+1).Phi_Z1;
            Phi_Z2       = Tr.CVRods{i}(j+1).Phi_Z2;
        else %order 2
            Phi_Z        = Tr.CVRods{i}(j+1).Phi_Z;
        end
