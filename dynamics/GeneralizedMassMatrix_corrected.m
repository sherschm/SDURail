...
function M = GeneralizedMassMatrix_corrected(Tr,q)
...
for i = 1:N
    ...
    for j = 1:Tr.VLinks(Tr.LinkIndex(i)).npie-1 %will run only if soft link
        ...
        Ms       = Tr.CVRods{i}(j+1).Ms;
        Xs       = Tr.CVRods{i}(j+1).Xs;
        Ws       = Tr.CVRods{i}(j+1).Ws;
        ...
        for ii = 2:nip
            ...
            if Tr.Z_order==4
                ...
                B_Z1here  = Tr.CVRods{i}(j+1).Phi_Z1(6*(ii-2)+1:6*(ii-1),:);%note this step
                B_Z2here  = Tr.CVRods{i}(j+1).Phi_Z2(6*(ii-2)+1:6*(ii-1),:);
                if dof_here>0
                ...
                B_Zhere  = Tr.CVRods{i}(j+1).Phi_Z(6*(ii-2)+1:6*(ii-1),:);%note this step
                if dof_here>0
